        ...
        <div class="flex items-center space-x-6">
          ...
          <div v-if="isLoggedIn" class="relative">
            <button class="flex items-center text-gray-600 hover:text-indigo-600 transition duration-300" @click="toggleProfileMenu">
              <img src="https://www.gravatar.com/avatar?d=mp" alt="User avatar" class="w-10 h-10 rounded-full border border-gray-300 shadow-sm">
              <svg v-if="!showProfileMenu" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" class="w-5 h-5 ml-2 text-gray-500">
            ...
            <div v-if="showProfileMenu" class="absolute right-0 mt-2 w-48 bg-white border border-gray-200 rounded-md shadow-lg z-10">
              <button class="block w-full text-left px-4 py-2 text-gray-600 font-medium hover:bg-gray-100 transition duration-200" @click="viewProfile">
                View Profile
              </button>
              <button class="block w-full text-left px-4 py-2 text-gray-600 font-medium hover:bg-gray-100 transition duration-200" @click="logout">
                Logout
              </button>
            </div>
          </div>
...
export default {
  setup() {
    ...
    onMounted(() => {
      if (!localStorage.getItem('guestMode') && !isLoggedIn.value) {
    ...
    // Close modal and handle guest or user actions
